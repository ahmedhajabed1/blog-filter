jQuery(document).ready(function($) {
    ...
    const $wrapper = $('#ajax-post-filter-wrapper');
    ...
    function loadPosts(appendMode = false) {
        ...
        const postsPerPage = $wrapper.data('posts-per-page') || 9;
        ...
        // AJAX request
        $.ajax({
            url: ajaxPostFilter.ajaxurl,
            type: 'POST',
            data: {
                action: 'filter_posts',
                nonce: ajaxPostFilter.nonce,
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (appendMode) {
                        ...
                        // Animate new posts
                        $('.post-item').slice(-response.data.posts.length).each(function(index) {
    ...
    $(document).on('click', '.remove-filter', function() {
        ...
        $(`input[name="category_filter"][value="${categoryId}"]`).prop('checked', false);
        updateActiveFilters();
        currentPage = 1;
        loadPosts();
    });
    ...
    // Optional: Auto-apply filters on checkbox change (comment out if you want manual apply only)
    /*
    $('input[name="category_filter"]').on('change', function() {
        currentPage = 1;
        loadPosts();
