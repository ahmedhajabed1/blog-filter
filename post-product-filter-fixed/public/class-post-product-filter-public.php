<?php
/**
 * Public functionality - SECURITY HARDENED v1.1.0
 * - FIXED: Passes preset-slug to JavaScript
...
class Post_Product_Filter_Public {
    ...
    public function enqueue_public_assets() {
        ...
        wp_localize_script('post-product-filter-public', 'postProductFilter', array(
            ...
            'nonce' => wp_create_nonce(POST_PRODUCT_FILTER_AJAX_FILTER_NONCE)
        ));
    }
    ...
    public function shortcode_handler($atts) {
        ...
        if (empty($slug)) {
            if (current_user_can('manage_options')) {
                return '<div class="post-product-filter-error" style="padding: 20px; background: #f8d7da; border: 1px solid #dc3545; border-radius: 4px; color: #721c24;">
                    <p style="margin: 0;"><strong>Error:</strong> No preset slug specified.</p>
                    <p style="margin: 10px 0 0 0; font-size: 14px;">Usage: <code>[post_product_filter slug="your-preset-slug"]</code></p>
                </div>';
            }
            return '';
        }
        ...
        if (!$preset) {
            // Don't expose available presets to non-admin users (security)
            if (current_user_can('manage_options')) {
                $available = !empty($presets) ? array_keys($presets) : array();
                return '<div class="post-product-filter-error" style="padding: 20px; background: #f8d7da; border: 1px solid #dc3545; border-radius: 4px; color: #721c24;">
                    <p style="margin: 0;"><strong>Error:</strong> Preset "' . esc_html($slug) . '" not found.</p>
                    ' . (!empty($available) ? '<p style="margin: 10px 0 0 0; font-size: 14px;">Available presets: ' . esc_html(implode(', ', $available)) . '</p>' : '') . '
                </div>';
            } else {
                return '';
            }
    ...
    public function render_filter($preset = null, $preset_slug = '') {
        if (!$preset || !isset($preset['settings']) || empty($preset_slug)) {
            if (current_user_can('manage_options')) {
                echo '<div class="post-product-filter-error" style="padding: 20px; background: #f8d7da; border: 1px solid #dc3545; border-radius: 4px; color: #721c24;">',
                     '<p style="margin: 0;">', esc_html__('Invalid preset configuration.', 'post-product-filter'), '</p>',
                     '</div>';
            }
            return;
        }
                ...
                <div class="filter-sidebar">
                    <div class="filter-widget">
                        ...
                        <div class="filter-search">
                            <input type="text" id="category-search" placeholder="<?php esc_attr_e('Search categories...', 'post-product-filter'); ?>" aria-label="<?php esc_attr_e('Search categories', 'post-product-filter'); ?>">
                        </div>
                        <?php endif; ?>
                        
                        <div class="filter-options" role="group" aria-label="<?php esc_attr_e('Category filters', 'post-product-filter'); ?>">
                            <?php
                            ...
                            if ($preset_type === 'products' && class_exists('WooCommerce')) {
                                $categories = get_terms(array(
                                    ...
                                    'number' => 500 // Limit to prevent DoS
                                ));
                            } else {
                            ...
                            if (!is_wp_error($categories) && !empty($categories)) {
                                ...
                                echo '<p style="padding: 10px; color: #666;">' . esc_html__('No categories available.', 'post-product-filter') . '</p>';
                            }
                            ?>
                        </div>
                    </div>
                    
                    <div class="active-filters" style="display: none;" role="region" aria-label="<?php esc_attr_e('Active filters', 'post-product-filter'); ?>">
                ...
                <div class="posts-container">
                    <div class="posts-header">
                        <div class="results-count" role="status" aria-live="polite">
                            ...
                            <?php echo $preset_type === 'products' ? esc_html__('products', 'post-product-filter') : esc_html__('posts', 'post-product-filter'); ?>
                        </div>
                        
                        <div class="loading-overlay" style="display: none;" role="status" aria-live="assertive" aria-label="<?php esc_attr_e('Loading content', 'post-product-filter'); ?>">
                            <div class="loader"></div>
                            <p class="loading-text"><?php echo esc_html($loading_text); ?></p>
                        </div>
                    </div>
                    
                    <div id="posts-grid" class="posts-grid" style="grid-template-columns: repeat(<?php echo esc_attr($columns); ?>, 1fr);" role="region" aria-live="polite">
                        <!-- Posts will be loaded here via AJAX -->
                    </div>
                    
                    <div id="posts-pagination" class="posts-pagination" role="navigation" aria-label="<?php esc_attr_e('Pagination', 'post-product-filter'); ?>">
    ...
    public function add_seo_meta() {
        ...
        $paged = max(1, min(1000, $paged)); // Bound pagination
